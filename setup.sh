...
if [ "$RECONFIGURE" -eq 1 ] || [ ! -f "$GIT_IDENTITY" ]; then
  ...
  if [ "$ASSUME_YES" -eq 1 ] || [ ! -t 0 ]; then
    ...
    printf "\033[1;36m??\033[0m Your name for git commits%s: " "${CUR_NAME:+ [$CUR_NAME]}"
    read -r GIT_NAME; GIT_NAME="${GIT_NAME:-$CUR_NAME}"
    printf "\033[1;36m??\033[0m Your email for git commits%s: " "${CUR_EMAIL:+ [$CUR_EMAIL]}"
    read -r GIT_EMAIL; GIT_EMAIL="${GIT_EMAIL:-$CUR_EMAIL}"
  fi
  ...
  if [ -x "$OP_SSH_SIGN" ] && [ -t 0 ] && [ "$ASSUME_YES" -eq 0 ]; then
    if ask "Sign commits with your 1Password SSH key (shows 'Verified' on GitHub)?"; then
      printf "\033[1;36m??\033[0m Public signing key (ssh-ed25519 AAAA…)%s: " "${CUR_KEY:+ [keep current]}"
      read -r GIT_SIGNKEY; GIT_SIGNKEY="${GIT_SIGNKEY:-$CUR_KEY}"
    fi
  elif [ -n "$CUR_KEY" ]; then
  ...
  mkdir -p "$(dirname "$GIT_IDENTITY")"
  {
    ...
    echo "# Included from dotfiles/gitconfig. Edit freely, or re-run:"
    echo "#   bash ~/mac-setup/setup.sh --reconfigure"
    echo "[user]"
    [ -n "$GIT_NAME" ]  && printf "\tname = %s\n" "$GIT_NAME"
    [ -n "$GIT_EMAIL" ] && printf "\temail = %s\n" "$GIT_EMAIL"
    if [ -n "$GIT_SIGNKEY" ]; then
      printf "\tsigningkey = %s\n" "$GIT_SIGNKEY"
      echo "[gpg]"
      echo "	format = ssh"
      echo '[gpg "ssh"]'
