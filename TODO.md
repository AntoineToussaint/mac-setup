...
- [ ] **Keep the tunnels running across reboots** — both public endpoints are
      live but depend on a foreground process plus `warden` on :18982:
        * `https://gateway.<your-domain>` — Cloudflare named tunnel (no auth)
        * `https://<your-machine>.<your-tailnet>.ts.net` — Funnel (no auth)
      ...
      (Zero Trust → Access → Applications) does the same for the Cloudflare one.
- [ ] **Delete the stale `dev` CNAME** — Cloudflare dashboard → your zone →
      DNS → Records. The old `dev.<your-domain>` tunnel is gone, but cloudflared
...
- [ ] **Tailscale Funnel** — app installed and logged in as
      `<your-machine>.<your-tailnet>.ts.net` (doctor confirms). Remaining:
