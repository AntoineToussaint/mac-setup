...
2. **GitHub** — signs you in with `gh auth login`, requesting the `write:public_key` and
   `admin:ssh_signing_key` scopes up front so the SSH key below can be registered in the same pass.
3. **Dotfiles** — symlinks `dotfiles/*` into `~` (existing real files are backed up to `*.bak-<timestamp>`).
   ...
   Commit signing uses a **plain SSH key** — git signs with `ssh-keygen` itself, no agent and
   no external signer. If you have no key it offers to generate an ed25519 one, stores the
   passphrase in the Keychain, seeds `~/.ssh/allowed_signers` so local verification works,
   and adds the key to GitHub with `gh` as **both** an authentication and a signing key —
   register only the first and pushes work while commits still read "Unverified".
   1Password's `op-ssh-sign` is offered as an alternative; Obin does not use 1Password, so
   it is opt-in and needs a key already in your account.
4. **Runtimes** — installs and upgrades node/python/go/rust via [mise](https://mise.jdx.dev), plus rustup components and Go's `air`.
5. **Nix** — Determinate Systems installer / upgrade (not via Homebrew).
6. **Security** — runs [`security.sh`](security.sh): firewall + stealth, auto-updates, npm `ignore-scripts`, Touch ID for sudo (skip with `--no-security`).
7. **Verification** — runs [`doctor.sh`](doctor.sh) and exits non-zero if a machine-checkable requirement fails.
...
| Layer | What | How |
|---|---|---|
| Credentials | Signed commits, passphrase in the Keychain | Plain SSH key (ed25519), generated by `setup.sh` if absent and registered on GitHub via `gh`; passphrase held by the macOS Keychain. 1Password's `op-ssh-sign` optional. The key path lives in the untracked `~/.config/git/identity`, never in `dotfiles/gitconfig`. |
